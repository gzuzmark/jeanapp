...
class ProductList extends Component {
	...
	filterVisits(item) {
		const isSalesman =
			item.salesmanName.toLowerCase().indexOf(this.state.salesman.toLowerCase()) !== -1;
		const isCompany =
			item.clientName.toLowerCase().indexOf(this.state.companyName.toLowerCase()) !== -1;
		const isState =
			item.visitState.toLowerCase().indexOf(this.state.visitState.toLowerCase()) !== -1;
